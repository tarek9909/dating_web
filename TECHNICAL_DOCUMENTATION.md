# Digital Invitation Platform — Complete Technical Documentation & Architectural Analysis

**Application:** Digital Invitation Platform (Evolved from Dana's Date Invitation)  
**Frontend:** React 19.2.8 + Vite 8.2.2 (Dynamic Renderer Architecture)  
**Backend:** Node.js + Express.js REST API  
**Database:** MySQL 8+ via `mysql2/promise` (22 Tables)  
**Authentication:** JWT Access Tokens (15m) + HTTP-Only Refresh Sessions (14d)  
**Password Security:** Argon2id (with Bcrypt fallback)  
**Payment Model:** Manual Admin Verification & Atomic Account Activation  
**Date:** September 8, 2026  

---

## Table of Contents
1. [Executive Technical Assessment](#1-executive-technical-assessment)
2. [Existing Architecture Map](#2-existing-architecture-map)
3. [Existing Invitation Workflow](#3-existing-invitation-workflow)
4. [Existing vs Required Gap Matrix](#4-existing-vs-required-gap-matrix)
5. [Hardcoded Data Migration Inventory](#5-hardcoded-data-migration-inventory)
6. [Database Assessment & MySQL Schema](#6-database-assessment--mysql-schema)
7. [Backend Architecture Assessment](#7-backend-architecture-assessment)
8. [Frontend Integration & Renderer Architecture](#8-frontend-integration--renderer-architecture)
9. [Complete API Specification](#9-complete-api-specification)
10. [Security & Data Protection Assessment](#10-security--data-protection-assessment)
11. [Technical Debt & Risk Management](#11-technical-debt--risk-management)
12. [Phase-by-Phase Implementation Plan (Phases 0–18)](#12-phase-by-phase-implementation-plan-phases-018)
13. [Dependencies & Critical Path Graph](#13-dependencies--critical-path-graph)
14. [Final V1 Completion Checklist](#14-final-v1-completion-checklist)
...
The repository at `c:/Projects/dana` is an interactive single-page web application originally tailored as an affectionate, humorous date proposal for **Dana**. It is built with **React 19.2.8** and **Vite 8.2.2**. 

It currently possesses:
* **No Backend**: All interactions run purely in the browser.
* **No Database**: All choices, options, recipient data, and phone numbers are hardcoded.
...
## 6. Database Assessment & MySQL Schema
...
## 7. Backend Architecture Assessment
...
## 8. Frontend Integration & Renderer Architecture
...
## 10. Security & Data Protection Assessment
...
## 12. Phase-by-Phase Implementation Plan (Phases 0–18)
...
## 13. Dependencies & Critical Path Graph
...
### Critical Rationale for Sequence:
1. **Database & Auth (Phases 1-2) must precede business logic**: Everything depends on user roles, customer identity, and database connection pooling.
2. **Payment & Activation (Phases 4-6) must precede Invitation Builder**: A customer account and their initial invitation are only created when an admin marks the payment as paid and triggers activation.
3. **Public APIs (Phase 9) must precede Frontend Renderer Wiring (Phase 11)**: The dynamic renderer requires the backend JSON contract to exist before replacing hardcoded values.
4. **Renderer (Phase 11) must be tested before Builder (Phase 14)**: The builder relies on the renderer for live draft previewing.
...
- [x] Visitor can view landing page highlights.
- [x] Visitor can submit invitation request form (`name`, `phone`, `email`, `recipientName`, `notes`).
- [x] Request is validated via Zod and saved to `customers` and `customer_requests`.
- [x] Rate limiting blocks spam submissions (>5 per 15 minutes per IP).
- [x] Admin notification is queued in `email_outbox`.

### Manual Payment & Activation
- [x] Admin can view request in Admin Portal.
- [x] Admin can record manual payment details (`amount`, `payment_method`, `reference_number`).
- [x] Unpaid request cannot be activated.
- [x] Admin can trigger atomic activation:
  - [x] Customer user account created in `users`.
  - [x] Secure temporary password generated.
  - [x] Initial invitation instantiated in `invitations` and `invitation_content` from default template.
  - [x] Request status updated to `account_created`.
  - [x] Action logged in `audit_logs`.
  - [x] Entire operation aborts and rolls back if any query fails.

### Authentication & Security
- [x] Customer logs in with temporary password.
- [x] Forced password change screen intercepts login (`must_change_password = true`).
- [x] Customer submits new password; flag cleared; redirected to dashboard.
- [x] JWT access token (15m) and HTTP-only refresh cookie (14d) issued.
- [x] Token refresh rotation works; revoked sessions rejected.
- [x] Customer cannot access another customer's invitation data (ownership enforced).
- [x] Suspended customers are immediately blocked by auth middleware.
- [x] Admin routes are strictly protected against non-admin roles.

### Invitation Builder & Preview
- [x] Customer can edit recipient name, opening text, question, and buttons.
- [x] Customer can select locations from library or add custom descriptions.
- [x] Customer can select food options from library.
- [x] Customer can configure date (tomorrow, exact, custom) and time.
- [x] Customer can configure dress code and checklist items.
- [x] Customer can choose GIFs from library or upload private images/GIFs.
- [x] Customer can preview invitation in live preview mode (authenticated preview works on drafts).

### Publishing & Public Distribution
- [x] Publishing validates all required fields (recipient, question, locations, foods).
- [x] Customer can publish invitation; status becomes `published` with unique slug.
- [x] Public slug URL `/d/:slug` renders the complete romantic date invitation experience.
- [x] Visual design, glassmorphism, animations, canvas particles, Web Audio sounds, confetti, runaway NO button, and AngryModal work identically to the original app.
- [x] RSVP ticket displays correct selections and ticket ID.
- [x] "Send Confirmation on WhatsApp" button opens `wa.me` link with customer's phone number.
- [x] Draft or unpublished invitations return `404 Not Found` when accessed via `/d/:slug`.
- [x] Customer can unpublish invitation; public access is revoked immediately.

### Analytics & Administration
- [x] Page views, button clicks, card selections, shares, and RSVP completions logged to `invitation_events`.
- [x] Customer can view analytics counts in their dashboard.
- [x] Admin can manage templates, themes, locations, food options, and GIF library.
- [x] Admin can suspend/reactivate customer accounts or reset passwords.
